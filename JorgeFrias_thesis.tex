...
In this thesis, I study whether a set of risk factors explain the
variation in cryptocurrency returns and their compensation for risk
exposure. Using the Instrumented Principal Component Analysis (IPCA)
model of Kelly et al.
(\citeproc{ref-kellyCharacteristicsAre2019}{2019}), I estimate latent
factor models from a large panel of cryptocurrency characteristics that
instrument the unobservable, time-varying loadings. I show that the IPCA
performs better both in-sample and out-of-sample compared to traditional
static models with observable risk factors. The results indicate that a
two-factor IPCA model captures a substantial share of risk compensation,
while models with additional latent factors provide a more complete
description of return dynamics. The estimated factors are mainly linked
to short-term trading activity, volatility, and return distribution,
while others are associated with a mispricing component and market
exposure. Allowing for anomaly intercepts improves the in-sample fit but
adds no predictive power, as confirmed by asset-pricing tests. Overall,
the findings suggest that IPCA can effectively filter out irrelevant
information, extract systematic sources of risk from a large set of
characteristics, and provide a robust framework for modeling
...
(\citeproc{ref-kellyCharacteristicsAre2019}{2019}), since the IPCA model
...
three-factor model of Y. Liu et al.
(\citeproc{ref-liuCommonRisk2022}{2022}), an analogue of the Fama-French
three-factor model (\citeproc{ref-famaCommonRisk1993}{Fama \& French,
1993}). The fourth and fifth model add liquidity (LIQ) and volatility
(VOL), respectively.

Following Kelly et al.
(\citeproc{ref-kellyCharacteristicsAre2019}{2019}), I estimate two
...
the literature (\citeproc{ref-liuCommonRisk2020}{W. Liu et al., 2020};
\citeproc{ref-liuCommonRisk2022}{Y. Liu et al., 2022}), the momentum
...
\pandocbounded{\includegraphics[keepaspectratio]{pictures/gammas_K2.png}}

}

\caption[Characteristic contribution in 2-factor
model]{\label{fig-gammasK2}\textbf{Characteristic contribution in the
2-factor IPCA model.} The figure shows the \(\Gamma_\beta\) coefficient
estimates from the model with \(K=2\) factors. Characteristics are
grouped by category, with each category shown in a different color. The
model is estimated without an intercept and at the individual
cryptocurrency level.}

\end{figure}%

Figure~\ref{fig-gammasK4} reports the \(\Gamma_\beta\) coefficient
estimates for the \(K=4\) model. Factor 1 again loads strongly on the
CAPM alpha, similar to the second factor in the two-factor model. Factor
2 and 3 share common contributions from volatility, trading activity,
and distribution-related characteristics. Factor 4 loads heavily on CAPM
beta, giving it the interpretation of a market factor. Across both
specifications, the contribution of price-size and liquidity
characteristics is very limited, playing only a minor role in
determining the risk factor loadings.

\begin{figure}[h]

\centering{

\pandocbounded{\includegraphics[keepaspectratio]{pictures/gammas_K4.png}}

}

\caption[Characteristic contribution in 4-factor
model]{\label{fig-gammasK4}\textbf{Characteristic contribution in the
4-factor IPCA model.} The figure shows the \(\Gamma_\beta\) coefficient
estimates from the model with \(K=4\) factors. Characteristics are
grouped by category, with each category shown in a different color. The
model is estimated without an intercept and at the individual
cryptocurrency level.}
...
\subsubsection{Significant characteristics. Which ones
matter?}\label{significant-characteristics.-which-ones-matter}
...
more potential characteristics exist (see
\citeproc{ref-bianchiMispricingRiskCompensation2021}{Bianchi \& Babiak,
2021b}; \citeproc{ref-liuCommonRisk2022}{Y. Liu et al., 2022};
\citeproc{ref-mercikCrosssectionalInteractionsCryptocurrency2025}{Mercik
et al., 2025}). Kelly et al.
(\citeproc{ref-kellyCharacteristicsAre2019}{2019}) introduce a
...
(\citeproc{ref-bianchiMispricingRiskCompensation2021}{2021b}), who also
...
(\citeproc{ref-kellyCharacteristicsAre2019}{2019}), I test whether
...
al. (\citeproc{ref-kellyCharacteristicsAre2019}{2019}), who demonstrate
...
\(\Gamma_\alpha = \mathbf{0}\). The null hypothesis is that the
...
(\citeproc{ref-kellyCharacteristicsAre2019}{2019}) has been widely used
...
Consistent with Bianchi \& Babiak
(\citeproc{ref-bianchiMispricingRiskCompensation2021}{2021b}), I find
...
(\citeproc{ref-kellyCharacteristicsAre2019}{2019}), my results show that
...
\bibitem[\citeproctext]{ref-openai2025chatgpt}
...
(\citeproc{ref-liuCommonRisk2022}{2022}), who develop a 3-factor model
for the cryptocurrency market analogous to the Fama--French three-factor
model in equities Fama \& French
(\citeproc{ref-famaCommonRisk1993}{1993}). In addition, I include a
liquidity and a volatility factor, based on Bianchi \& Babiak
(\citeproc{ref-bianchi2021factor}{2021a}) and Lan \& Frömmel
(\citeproc{ref-lanRiskFactors2025}{2025}).
...
Novomestky, 2022}), PerformanceAnalytics (v. 2.0.8;
...
Additionally, I used Python (v. 3.15.3; \citeproc{ref-python}{Python
Software Foundation, 2025}) and the following packages: numpy (v. 2.2.6;
\citeproc{ref-harris2020array}{Harris et al., 2020}), pandas (v. 2.3.1;
\citeproc{ref-reback2020pandas}{The pandas development team, 2020}),
matplotlib (v. 3.10.3; \citeproc{ref-Hunter:2007}{Hunter, 2007}), and
scipy (v. 1.15.3; \citeproc{ref-2020SciPy-NMeth}{Virtanen et al.,
2020}), ipca (\citeproc{ref-ipca}{Buechner \& Bybee, 2019}), and Seth
Pruitt's code available at https://sethpruitt.net/research/.

ChatGPT (\citeproc{ref-openai2025chatgpt}{2025}) was used in the thesis
